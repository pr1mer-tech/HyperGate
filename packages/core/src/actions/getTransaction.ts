...
export interface TxRequest extends Omit<BaseRequest, "command"> {
  /**
...
export type GetTransactionParameters<config extends Config = Config> = Compute<
  TxRequest & ChainIdParameter<config>
>;
...
export interface BaseTxResult<
  Version extends APIVersion = typeof RIPPLED_API_V2,
...
export type GetTransactionReturnType = Compute<TxResponse>;

export type GetTransactionErrorType = ErrorType;

export async function getTransaction<config extends Config>(
  config: config,
  ...
  return res as GetTransactionReturnType;
}
